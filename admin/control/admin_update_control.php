...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($fullName) || empty($username) || empty($email) || empty($phoneNumber) || empty($role) || empty($address) || empty($securityQuestion) || empty($defaultLanguage) || empty($timeZone)) {
    ...
    $stmt = $connection->prepare("UPDATE admin SET full_name = ?, username = ?, email = ?, phone_number = ?, role = ?, address = ?, security_question = ?, default_language = ?, time_zone = ? WHERE AdminId = ?");
